...
server.init = function (options) {
  ...
  this.options.preScripts = this.options.preScripts || [];
  this.options.preScripts = this.options.preScripts.map((script) => {
    return fs.readFileSync(path.resolve(script), 'utf8');
  });
  const scriptDir = path.join(__dirname, './scripts');
  fs.readdirSync(scriptDir).forEach((file) => {
    this.options.preScripts.push(fs.readFileSync(path.join(scriptDir, file), 'utf8'));
  });
...
server.onRequest = function (req, res) {
  ...
  this.firePluginEvent('requestReceived', req, res)
    .then(() => {
      ...
      if (req.prerender.proxyServer) {
        try {
          new url.URL(req.prerender.proxyServer);
        } catch (error) {
          util.log('Invalid proxy URL:', req.prerender.proxyServer);
          req.prerender.statusCode = 400;
          req.prerender.statusCodeReason = 'Invalid proxy URL';
          return Promise.reject('Invalid proxy URL');
        }
      }
    ...
    .then(() => {
      ...
      var preScripts = this.options.preScripts;
      if (req.prerender.preScript) {
        req.prerender.preScript.split(',').forEach((script) => {
            preScripts.push(fs.readFileSync(path.join(__dirname + "/../", script), 'utf8'));
        });
      }
