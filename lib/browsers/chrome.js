  ...
  return new Promise((resolve, reject) => {
    ...
    connectToBrowser(
      ...
      .then((chromeBrowser) => {
        ...
        var browserOptions = { disposeOnDetach: true};
        if(options.proxyServer) {
          var proxyProcessed = processProxy(options.proxyServer);
          browserOptions.proxyServer = proxyProcessed.proxy;
        }
        if (proxyProcessed.proxyCredentials) {
          browser.Fetch.enable({ handleAuthRequests: true, patterns: [{ urlPattern: '*' }] }).then(() => {
            browser.Fetch.requestPaused(async (request) => {
                browser.Fetch.continueRequest({ requestId: request.requestId });
            });
            browser.Fetch.authRequired(async (params) => {
                browser.Fetch.continueWithAuth({
                  requestId: params.requestId,
                  authChallengeResponse: {
                    response: 'ProvideCredentials',
                    username: proxyProcessed.proxyCredentials.username,
                    password: proxyProcessed.proxyCredentials.password,
                  },
                });
            });
          });
        }
        return browser.Target.createBrowserContext(browserOptions);
      })
      .then(({ browserContextId }) => {
        ...
        return browser.Target.createTarget(targetOptions);
      })
      .then(({ targetId }) => {
...
chrome.loadUrlThenWaitForPageLoadEvent = function (tab, url, onNavigated, preScripts = []) {
  return new Promise((resolve, reject) => {
...
function processProxy(proxy) {
  if (proxy) {
    ...
    let proxyPart = proxy.split("//")[1] || proxy;
    let proxyCredentials = null;

    if (proxyPart.includes("@")) {
      const [credentialsPart] = proxyPart.split("@");
    ...
    const finalProxy = `${proxyUrl.protocol}//${proxyUrl.hostname}:${proxyUrl.port}`;
    return {proxy: finalProxy, proxyCredentials};
  }
  return {proxy: null, proxyCredentials: null};
}
